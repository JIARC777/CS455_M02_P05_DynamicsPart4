...
public class ObstacleAvoidance: Seek
{
    ...
    public override SteeringOutput GetSteering()
    {
        RaycastHit hit;
        SteeringOutput result = new SteeringOutput();
        if (Physics.Raycast(ai.transform.position, ai.transform.TransformDirection(Vector3.forward), out hit, lookAhead)) {
           // Debug.Log("Impending Collision Detected");
            target = new GameObject();
            Debug.DrawRay(ai.transform.position, ai.transform.TransformDirection(Vector3.forward) * hit.distance);
